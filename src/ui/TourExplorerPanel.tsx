    ...
    <div className="tour-tree-node-group">
      ...
      >
        {Array.from({ length: depth }).map((_, i) => (
          <span key={i} className="tour-tree-indent-guide" style={{ left: `${12 + i * 12 + 2}px` }} />
        ))}
      ...
      {contextMenu && (
        <>
          ...
          <div className="tour-tree-context-menu" style={{ top: contextMenu.y, left: contextMenu.x }}>
            {!isFolder && (
      ...
      {isFolder && expanded && node.children && (
        <div className="tour-tree-children">
          ...
          {node.children.length === 0 && (
            <p className="tour-tree-label opacity-40 italic" style={{ paddingLeft: `${8 + (depth + 1) * 12 + 22}px` }}>Empty</p>
          )}
        </div>
      )}
    </div>
