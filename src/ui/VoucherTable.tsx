...
export function VoucherTable({
  ...
  return (
    <section
      className={`app-panel app-panel-body-lg border transition-colors duration-300 ${isContentComplete ? "border-emerald-500/20 bg-emerald-50/[0.02]" : "border-amber-500/20 bg-amber-50/[0.02]"}`}
    >
      <div className="mb-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h3 className="app-section-title m-0">Voucher Content</h3>
          {isContentComplete !== undefined && (
            <SectionIndicator
        ...
        <table className="w-full min-w-[1750px] table-fixed border-collapse text-sm">
          ...
          <thead>
            <tr className="border-y border-line bg-cloud text-left text-xs font-bold uppercase tracking-wide text-steel">
              ...
              <th
                className="px-2 py-3 text-center border-r border-line"
                colSpan={4}
              >
                Rooms
              </th>
            ...
            {fields.map((field, index) => (
              <tr key={field.id}>
                {lineItemColumns.map((column) => (
                  <td
                    className={`px-2 py-2 border-r border-line ${column.className}`}
                    key={column.name}
                  >
                      ...
                      <Controller
                        ...
                        render={({ field: controllerField }) => {
                          ...
                          const rowOpts = availableSupplements
                            .filter(
                              (s) =>
                                s.room_category.toLowerCase() ===
                                cat.toLowerCase(),
                            )
                            .map((s) => ({
                              name: s.supplement_name,
                              label: `${s.supplement_name} (${s.supplement_amount})`,
                            }));
                          return (
                            ...
                            <input
                              ...
                              ref={(el) => {
                                ...
                                if (el && roomCountFields.has(column.name)) {
                                  ...
                                  const elWithWheel = el as HTMLInputElement & {
                              ...
                              type={column.type}
                              min={
                                roomCountFields.has(column.name) ? 0 : undefined
                              }
                              step={
                                roomCountFields.has(column.name) ? 1 : undefined
                              }
                              className={tableControlClass}
                              value={
                                roomCountFields.has(column.name) &&
        ...
        {dailyRooms.length > 0 ? (
          dailyRooms.map((dr, idx) => (
            <span key={idx} className="text-steel">
              {dr.date} rooms: <span className="text-navy">{dr.rooms}</span> /
              child: <span className="text-navy">{dr.children}</span>
            </span>
          ))
        ) : (
