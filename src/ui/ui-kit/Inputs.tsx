...
import React, { forwardRef, useState, useRef, useEffect, type ComponentPropsWithoutRef } from "react";
      ...
      <div className="relative w-full" ref={containerRef}>
        ...
        <div className="app-select-shell w-full" data-open={isOpen}>
          <button
            ...
            className={[
              ...
              isCompact ? "app-select-compact h-9 leading-[2.25rem] py-0" : "py-2.5 min-h-[42px]",
              props.disabled ? "opacity-50 cursor-not-allowed" : "hover:border-steel",
              className,
            ]
              .filter(Boolean)
              .join(" ")}
        ...
        {isOpen && !props.disabled && (
          <div className="absolute top-full left-0 mt-1 w-full bg-surface border border-line shadow-2xl rounded-app z-[200] max-h-60 overflow-y-auto dropdown-scrollbar p-1 animate-in fade-in slide-in-from-top-1 duration-150">
            {options.length === 0 ? (
              <div className="p-3 text-xs text-steel text-center select-none font-medium">No options available</div>
            ) : (
              options.map((opt) => {
