...
export function SavedVouchersScreen({
  ...
  const generatedCount = voucherRegister.filter(v => v.status === "generated").length;
  ...
  const setDateRange = (days: number | null) => {
    ...
    const nextFilters = { ...voucherFilters, dateFrom: formattedPast, dateTo: formattedToday };
    setVoucherFilters(nextFilters);
    void refreshVoucherRegister(nextFilters);
  };
    ...
    <div className="mx-auto max-w-[1500px] p-4 md:p-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-steel">Operations & Data Studio</p>
            <h2 className="mt-1 font-display text-3xl font-bold text-navy tracking-tight">Saved Vouchers</h2>
            <p className="mt-1 text-sm text-steel">Manage, track, and supervise all active bookings and historic revisions.</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <RefreshCw size={14} className={isLoadingRegister ? "animate-spin" : ""} />
            Reload List
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="relative overflow-hidden rounded-app border border-line bg-surface p-5 shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-steel">Total Records</span>
              <p className="text-2xl font-bold text-navy mt-0.5">{totalCount}</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-app border border-line bg-surface p-5 shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-steel">Draft State</span>
        ...
        <div className="relative overflow-hidden rounded-app border border-line bg-surface p-5 shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-steel">Generated</span>
              <p className="text-2xl font-bold text-navy mt-0.5">
                {generatedCount}
                <span className="text-xs font-normal text-steel ml-2">
                  ({totalCount ? Math.round((generatedCount / totalCount) * 100) : 0}%)
                </span>
              </p>
            </div>
          </div>
        ...
        <div className="relative overflow-hidden rounded-app border border-line bg-surface p-5 shadow-sm hover:shadow-md transition-all group">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-steel">Dispatched / Sent</span>
              <p className="text-2xl font-bold text-navy mt-0.5">
                {sentCount}
                <span className="text-xs font-normal text-steel ml-2">
      ...
      <div className="app-panel app-panel-body-lg border border-line shadow-sm rounded-app bg-surface">
        ...
        <div className="border-b border-line pb-6 mb-6">
          <div className="flex flex-col xl:flex-row gap-4 items-stretch xl:items-end justify-between">
            ...
            <form onSubmit={handleSearchSubmit} className="flex-1 min-w-[280px]">
              <div className="space-y-2">
                <span className="text-xs font-bold uppercase tracking-wider text-steel block">Search Vouchers</span>
            ...
            {/* Quick date presets */}
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase tracking-wider text-steel block">Quick Presets</span>
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-line/40">
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase tracking-wider text-steel block">Status Classification</span>
              <Select
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase tracking-wider text-steel block">Date Start</span>
              <div className="relative">
                <input
                  type="date"
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold uppercase tracking-wider text-steel block">Date End</span>
              <div className="relative">
                <input
                  type="date"
                  className="w-full rounded-app border border-line px-3 py-2 text-sm bg-cloud/20"
        ...
        {isLoadingRegister ? (
          <div className="py-20 text-center space-y-3">
            ...
            <p className="text-sm font-semibold text-steel">Retrieving data record matrix...</p>
          </div>
          ...
          <div className="py-16 text-center max-w-md mx-auto space-y-4">
            ...
            <div>
              <p className="text-base font-bold text-navy">No records matching parameters</p>
              <p className="text-sm text-steel mt-1">Try resetting the filters or typing a different search query in the search bar above.</p>
            </div>
            <button
              onClick={handleClearFilters}
              ...
              <table className="w-full min-w-[900px] border-collapse text-sm text-left">
                <thead>
                  <tr className="bg-cloud/70 border-b border-line text-xs font-bold uppercase tracking-wider text-navy">
                    <th className="px-5 py-4 w-[200px]">Requisition / Tour #</th>
                ...
                <tbody className="divide-y divide-line/60 bg-surface">
                  {voucherRegister.map((voucher) => {
                    ...
                    if (voucher.voucherType === "reservation") {
                      ...
                      typeClass = "bg-cyan-50 text-cyan-700 border border-cyan-100";
                    ...
                    let statusBg = "bg-slate-50 text-slate-600 border border-slate-200";
                    if (voucher.status === "draft") {
                      statusDot = "bg-amber-400";
                      statusBg = "bg-amber-50 text-amber-800 border border-amber-200";
                    } else if (voucher.status === "generated") {
                      statusDot = "bg-indigo-500";
                      statusBg = "bg-indigo-50 text-indigo-800 border border-indigo-200";
                    ...
                    return (
                      <tr 
                        ...
                        onDoubleClick={() => void openVoucherFromSearch(voucher)}
                      >
                        <td className="px-5 py-3.5">
                          <div className="flex flex-col gap-1">
                            <span className="font-bold text-navy hover:text-navy/80 transition-all text-sm">
                              {voucher.requisitionNo || voucher.tourNo || "No Ref"}
                            </span>
                            {voucher.tourName ? (
                              <span className="text-[11px] text-steel font-medium truncate max-w-[180px]" title={voucher.tourName}>
                                {voucher.tourName}
                              </span>
                            ) : (
                              <span className="text-[11px] text-slate-400 italic">No Tour Name</span>
                            )}
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="space-y-1">
                            <p className="font-semibold text-navy-light">{voucher.hotelName}</p>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${typeClass}`}>
                              {voucher.voucherType}
                            </span>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5">
                            <span className="font-medium text-slate-700">{voucher.customerName || "Walk-In Customer"}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="text-xs">
                            <p className="font-semibold text-navy-light">{new Date(voucher.createdAt).toLocaleDateString("en-GB", { day: '2-digit', month: 'short', year: 'numeric' })}</p>
                            <p className="text-[11px] text-steel">{new Date(voucher.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2">
                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${statusBg}`}>
                              <span className={`h-1.5 w-1.5 rounded-full ${statusDot} animate-pulse`} />
                              <span className="capitalize">{voucher.status}</span>
                            </div>
                            
                            <label className="sr-only" htmlFor={`status-changer-${voucher.id}`}>
                        ...
                        <td className="px-5 py-3.5 text-right">
                          ...
                          >
                            <Eye size={12} />
                            <span>{openingVoucherId === voucher.id ? "Loading..." : "Open"}</span>
                          </button>
                        </td>
                      </tr>
                    );
